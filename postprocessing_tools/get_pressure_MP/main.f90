...
open(1456,file='./output/integral.dat',status='new',form='formatted')
write(1456,'(a10,12(a15))') 'step','t^+','press diff c','visc diff c','visc diss c','prod c','turb diff c', &
 &                                 'press diff d','visc diff d','visc diss d','prod d','turb diff d','interf'
close(1456,status='keep')

do i=nstart,nend,delta
 call calc_pressure(i)
enddo
! run last step if not already run
if(mod(nend,delta).ne.0) call calc_pressure(nend)

...
subroutine fftx_fwd_2D(u,uc,nx,npy,aliasing)

use fftw3
implicit none

integer(c_int) :: nx,npy
integer :: aliasing

real(c_double) :: u(nx,npy),uc(nx/2+1,npy,2)
complex(c_double_complex) :: wt(nx/2+1,npy)


call fftw_execute_dft_r2c(plan_x_fwd_2D,u,wt)

uc(1:nx/2+1,1:npy,1)=dble(wt(1:nx/2+1,1:npy))
uc(1:nx/2+1,1:npy,2)=aimag(wt(1:nx/2+1,1:npy))

! dealiasing
if(aliasing.eq.1)then
 uc(floor(2.0/3.0*real(nx/2+1))+1:nx/2+1,1:npy,1)=0.0d0
 uc(floor(2.0/3.0*real(nx/2+1))+1:nx/2+1,1:npy,2)=0.0d0
endif

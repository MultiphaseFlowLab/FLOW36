...
double precision :: gradm,mean,rms,curv,kinval
...
phiy(:,:,ny)=(phi(:,:,ny)-phi(:,:,ny-1))/(y(ny)-y(ny-1))
do j=2,ny-1
  ...
  do k=1,nz
    do i=1,nx
      ...
      !if(abs(phi(i,k,j)).le.threshold/50.0d0)then
        count=count+1
        ! find psi value at that point
        call get_kin(i,k,j,kinval)
        write(75,'(2(es16.8))') kv(i,k,j),kinval
...
subroutine get_kin(i,k,j,kinval)

use commondata

integer :: i,j,k
double precision :: kinval

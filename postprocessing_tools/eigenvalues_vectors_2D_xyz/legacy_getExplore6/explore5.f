      SUBROUTINE EXPLORE(PHI, NX, NY, NZ, NXH,X,Y,Z,DX,DY,TIME)
c-----------------------------------------------------------------------
C-----A.Roccon 07/2015
C-----Subroutine explore the PHI field....and then counts the drop and calculate the medimum mass of the drops
      ...
      INTEGER DROPS,IM,IP,JM,JP
      DOUBLE PRECISION X(NX), Y(NY), Z(NZ) 
      DOUBLE PRECISION DV,VOL,VOLM,REQ,REQM
...
C      ALLOCATE(MAP(NX,NZ,NY))
C      print*,'Start to copy PHI in PHI EXPLORE'
...
C              print*,'erase PN'
              PN(:,:)=0
              ENDDO
C              print*,'exiting the loop of exploration of the -th drop'
              REQ=(4*VOL/(3*PI))**(1.0d0/3.0d0)
              print*,'DROP N',drops,'REQ',REQ,'VOL',VOL
        ...
        print*,'drops',drops
         ...
         OPEN(56,FILE='Drops.txt',POSITION='APPEND') 
         write(56,1007) TIME,Drops, VOLM, REQM
         close(56)
 1007 FORMAT(E14.8,3x,I8,3x,E14.8,3x,E14.8)

...
subroutine lagran4_T(ppos,ptemp)
...
double precision, dimension(3) :: ppos
...
do j=1,5
 do k=1,5
  do i=1,5
   ptemp=ptemp+Tf(iarr(i),karr(k),jarr(j))*pol(i,1)*pol(j,2)*pol(k,3)
  enddo
 enddo
enddo
#else
...
do j=1,5
 do k=1,5
   ptemp=ptemp+Tf(i2,karr(k),jarr(j))*pol(j,2)*pol(k,3)
 enddo
enddo
#endif

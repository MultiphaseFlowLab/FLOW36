...
double precision :: modnabphi,normflux
...
do j=1,fpy
  do k=1,fpz
    do i=1,nx
      modnabphi=sqrt(phinx(i,k,j)**2.0d0+phiny(i,k,j)**2.0d0+phinz(i,k,j)**2.0d0)
      phinx(i,k,j)=phinx(i,k,j)/modnabphi
      phiny(i,k,j)=phiny(i,k,j)/modnabphi
      phinz(i,k,j)=phinz(i,k,j)/modnabphi
    enddo
...
do j=1,fpy
  do k=1,fpz
    do i=1,nx
      ...
      a4f(i,k,j)=-lamphi/(sqrt(2.0d0)*ch)*a1f(i,k,j)*phinx(i,k,j) + normflux*phinx(i,k,j)
      a5f(i,k,j)=-lamphi/(sqrt(2.0d0)*ch)*a1f(i,k,j)*phiny(i,k,j) + normflux*phiny(i,k,j)
      a6f(i,k,j)=-lamphi/(sqrt(2.0d0)*ch)*a1f(i,k,j)*phinz(i,k,j) + normflux*phinz(i,k,j)
    enddo
  enddo
enddo
